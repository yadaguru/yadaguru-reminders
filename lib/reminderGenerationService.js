...
module.exports = function(config) {
  ...
  function groupAndSortByDueDate(reminders) {
    ...
    return reminders.reduce(function(groupedReminders, reminder) {
      ...
      var isLate = moment.utc(reminder.dueDate).isBefore(moment.utc(), 'day');

      if (isLate) {
        // skip this reminder if it is late and has no late message or detail
        if (!reminder.lateMessage || !reminder.lateMessage.trim() || 
            !reminder.lateDetail || !reminder.lateDetail.trim()) {
      ...
      currentGroup.reminders.push({
        ...
        dueDate: reminder.dueDate,
        message: isLate ? reminder.lateMessage: reminder.message,
        detail: isLate ? reminder.lateDetail : reminder.detail
      });
  ...
  function deDuplicateReminders(reminders) {
    ...
    var deDupedReminders = reminders.reduce(function(deDupedReminders, reminder) {
      ...
      if (previousReminder.baseReminderId === reminder.baseReminderId) {
